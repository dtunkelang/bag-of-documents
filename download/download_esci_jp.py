#!/usr/bin/env python3
"""Download and filter Amazon ESCI Japanese (jp locale) via HuggingFace tasksource/esci.

Writes (in esci_jp_data/):
  titles.json          — unique Japanese product titles
  product_ids.json     — parallel list of product_ids
  train_queries.jsonl  — Japanese train queries
  train_qrels.jsonl    — graded 0-3
  test_queries.jsonl   — Japanese test queries
...
print(f"\nTotal unique Japanese products: {len(all_products):,}")
...
with open(f"{OUT}/train_queries.jsonl", "w") as f:
    for qid, qtext in train_queries.items():
        f.write(json.dumps({"query_id": qid, "query": qtext}, ensure_ascii=False) + "\n")
