...
# ISO 3166-1 alpha-2 -> localized country name (CLDR via Babel). English lives JS-side as
# FACET_VALUE_LABELS.location_country (the fallback); only non-English overrides live here.
# location_state (US states) is NOT localized — its full names stay JS-side for every language.
_COUNTRY_VALS = {
    "fr": {
        "US": "États-Unis",
        "GB": "Royaume-Uni",
...
def _build_locale_data() -> dict:
    ...
    for lng in SITE_LANGS:
        ...
        entry["values"] = {
            ...
            "location_country": _COUNTRY_VALS.get(lng, {}),
...
const FACET_VALUE_LABELS = {
  ...
  // ISO 3166-1 alpha-2 -> English country name (the fallback; per-language names live in LOCALE.values).
  location_country: {
    US: 'United States', GB: 'United Kingdom', IN: 'India', SG: 'Singapore', CA: 'Canada', DE: 'Germany', BR: 'Brazil', FR: 'France', JP: 'Japan', AU: 'Australia', PH: 'Philippines', MX: 'Mexico', ES: 'Spain', KR: 'South Korea', PL: 'Poland', IT: 'Italy', IL: 'Israel', HK: 'Hong Kong', CN: 'China', NL: 'Netherlands', MY: 'Malaysia', NZ: 'New Zealand', IE: 'Ireland', TW: 'Taiwan', DK: 'Denmark', CO: 'Colombia', AR: 'Argentina', TH: 'Thailand', ZA: 'South Africa', VN: 'Vietnam', SE: 'Sweden', CH: 'Switzerland', AE: 'United Arab Emirates', ID: 'Indonesia', NO: 'Norway', PE: 'Peru', SA: 'Saudi Arabia', FI: 'Finland', CL: 'Chile',
  },
  // US state code -> full name. Not localized (US state names are stable across languages),
  // so this single map serves every site language via the FACET_VALUE_LABELS fallback.
  location_state: {
...
function repaintMaps() {
  ...
  if (mapField) {
    ...
    const selNames = sel.map(c => facetValueLabel(mapField, c));
    document.getElementById('map-sel').textContent = sel.length ? (sel.length + ' ' + t('map_selected') + ' ' + selNames.join(', ')) : t('map_none');
  }
}
