#!/usr/bin/env python3
"""Build the grounded German *related-searches* bundle (space/de_related.json).

German related searches can't ride the e5-small-v2 role suggester (it ranks German by
morphology, not meaning), and German has no national mobilite graph the way the French
ROME lane uses. The grounded substitute is the ESCO occupation backbone (esco_backbone.py):

  query --(ESCO German label)--> occupation --(shared-skill relatedness)--> related
        occupations --(corpus-mined German role)--> a German role that EXISTS in our
        inventory
...
occupations (the few hundred ESCO occupations we actually have German postings for) and
...
Consumed by suggest_lib.DeRelatedSuggester. Re-run after a corpus refresh (so display
labels track live inventory) or an ESCO version bump.

Usage:
  ./run.sh start                 # local Solr must be up on :8983
  .venv/bin/python build_de_related.py
"""
...
OUT = os.path.join(HERE, "space", "de_related.json")

sys.path.insert(0, HERE)
from esco_backbone import backbone, fold  # noqa: E402
from mine_de_roles import _STOP_FOLDED, clean_title, fetch_titles  # noqa: E402

# Feminine German occupational suffix -> masculine, applied to a FOLDED token, used only
# as a resolution fallback (ESCO carries most feminine alt labels directly, but corpus
# surfaces drift). '-innen' (plural) before '-in'; only fires when the stem stays >=4
# chars so short words ('berlin') aren't mangled.
_DE_FEM = [("innen", ""), ("in", "")]
...
def _resolve_uri(text: str, bb) -> str | None:
    """Folded-exact ESCO German occupation for a corpus role label, with a degendered
    retry. Returns one URI (lexicographic min on collision) or None."""
    uris = bb.resolve(text, "de")
    if not uris:
        dg = degender_de(fold(text))
        if dg != fold(text):
            uris = bb.resolve(dg, "de")
...
def main() -> None:
    ...
    # ---- 1. corpus-mined German role vocab (the grounded display labels) ----
    titles = fetch_titles()
    ...
    for t in titles:
        ...
        if not (2 <= len(c) <= 48) or len(c.split()) > 6:
            continue
        if not re.search(r"[a-zà-ÿäöüß]", c) or re.search(r"\d", c):
            continue
        k = fold(c)
    ...
    for g in grounded:
        for n, s in bb.related(g, top=24, min_overlap=0.15):
            if n == g:
                continue
    ...
    # ---- 3. label resolver: ESCO German labels (preferred + alt) for every occupation a
    # query can land on (grounded or reachable), so a typed German role resolves even when
    ...
    for uri in resolvable:
        r = bb.by_uri.get(uri, {})
        for lab in (r.get("preferredLabel") or {}).get("de", "").split("/"):
            if fold(lab):
                cand_labels[fold(lab)].add(uri)
        for a in (r.get("altLabels") or {}).get("de", []) or []:
            if fold(a):
