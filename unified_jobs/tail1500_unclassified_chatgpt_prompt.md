# ChatGPT prompt: label top-1500 unclassified employer slugs with industry

These are employer slugs that the TF-IDF propagation pipeline could not confidently classify after two rounds (round-2 result was `unclassified`). Most are smaller employers (1-100 postings each) where slug + title context is the only signal. Slug names are often very informative — e.g. `solarwinds`, `philzcoffeecareers`, `blue-origin`.

Upload `tail1500_unclassified_for_labeling.csv` to a ChatGPT conversation (recommend GPT-5 or GPT-4.1 with Code Interpreter / Advanced Data Analysis enabled), then paste the prompt below.
