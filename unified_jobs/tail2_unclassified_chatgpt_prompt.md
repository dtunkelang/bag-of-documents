# ChatGPT prompt: label next 1,500 unclassified employer slugs (batch 2) with industry

These are employer slugs that the TF-IDF propagation pipeline could not confidently classify after two rounds (round-2 result was still `unclassified`). Most are smaller employers (typically 1-50 postings each — already past the top-1500 covered in batch 1) where slug + title context is the only signal. Slug names are often very informative — e.g. `solarwinds`, `philzcoffeecareers`, `blue-origin`.

Upload `tail2_unclassified_for_labeling.csv` to a ChatGPT conversation (recommend GPT-5 or GPT-4.1 with Code Interpreter / Advanced Data Analysis enabled), then paste the prompt below.

---

I have uploaded a CSV with 1500 employer slugs from a job-postings corpus that an automated classifier could not label. Each row has:
