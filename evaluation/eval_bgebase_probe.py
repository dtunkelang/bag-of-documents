#!/usr/bin/env python3
"""Evaluate bge-base BoD encoder vs MiniLM-L6 baseline.

Tests the capacity hypothesis: does a 5x larger base (109M vs 22M) lift the
BoD-trained sumsim signal?

Setups:
  A0. base MiniLM as retriever                         (15.60 baseline)
  A1. 6M-MNRL MiniLM as retriever (rerank_A)           (19.83 baseline)
  K1. bge-base BoD as retriever                        (NEW)
  A2. BM25-top50 reranked by 6M-MNRL MiniLM single     (single-stream baseline)
  K2. BM25-top50 reranked by bge-base BoD single       (NEW)

Saves per-query R@10/E@1 to /tmp/bgebase_probe_per_query.json for bootstrap.
"""
...
def main():
    ...
    print(f"  6M-MNRL MiniLM (rerank_A): {time.time() - t0:.0f}s", flush=True)
    t0 = time.time()
    qv_k = encode_subproc(os.path.join(SCRIPT_DIR, "query_model_us_full_6m_mnrl_bgebase"), queries)
    print(f"  bge-base BoD (rerank_K): {time.time() - t0:.0f}s", flush=True)

    print("\nloading product matrices...", flush=True)
    pv_base = np.load(os.path.join(INDEX_DIR, "rerank_A.vecs.fp16.npy")).astype(np.float32)
    print(f"  rerank_A: {pv_base.shape}, dim={pv_base.shape[1]}", flush=True)
    pv_k = np.load(os.path.join(INDEX_DIR, "rerank_K_bgebase.vecs.fp16.npy")).astype(np.float32)
    print(f"  rerank_K: {pv_k.shape}, dim={pv_k.shape[1]}", flush=True)
    ...
    print(f"  bm25 top200 from {os.path.basename(bm25_top200_path)}", flush=True)
    bm25_top200 = np.load(bm25_top200_path)
    bm25_top50 = bm25_top200[:, :50]

    print("\nretrieval setups (top-100 then top-10)...", flush=True)
    print("  A0 (base MiniLM)...", flush=True)
    base_index_pos = brute_top_k(qv_base, pv_base, K_RETRIEVE)
    print("  A1 (6M-MNRL MiniLM)...", flush=True)
    a_index_pos = brute_top_k(qv_a, pv_base, K_RETRIEVE)
    print("  K1 (bge-base BoD)...", flush=True)
    k_index_pos = brute_top_k(qv_k, pv_k, K_RETRIEVE)

    print("\nrerank setups (BM25 top-50 reranked)...", flush=True)
    a_rerank_pos = rerank_top50(qv_a, pv_base, bm25_top50, K_EVAL)
    k_rerank_pos = rerank_top50(qv_k, pv_k, bm25_top50, K_EVAL)
    ...
    orderings = {
        "A0_base_minilm_retriever": to_pids(base_index_pos[:, :K_EVAL]),
        "A1_6m_mnrl_minilm_retriever": to_pids(a_index_pos[:, :K_EVAL]),
        "K1_bge_base_bod_retriever": to_pids(k_index_pos[:, :K_EVAL]),
        "A2_bm25top50_minilm_rerank": to_pids(a_rerank_pos),
        "K2_bm25top50_bgebase_rerank": to_pids(k_rerank_pos),
    }
    ...
    out_path = "/tmp/bgebase_probe_per_query.json"
